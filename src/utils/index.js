...
/* ══════════════════════════════════════════════════════
   企業名 表示用クリーニング（類似判定専用）

   ① 法人格（前付き・後付き）を除去
   ② 全角・半角スペースを除去
   ※ 記号（・ など）は残す / 大文字小文字も区別しない
══════════════════════════════════════════════════════ */
export const stripCompany = (name) => {
  if (!name) return "";
  let s = String(name).trim();
  s = s.replace(/^(株式会社|有限会社|合同会社|一般社団法人|一般財団法人|公益社団法人|公益財団法人|医療法人|学校法人|社会福祉法人|特定非営利活動法人|ＮＰＯ法人|NPO法人|（株）|\(株\)|（有）|\(有\))/, "");
  s = s.replace(/(株式会社|有限会社|合同会社)$/, "");
  s = s.replace(/[\s　]/g, ""); // 全角・半角スペース除去
  return s.toLowerCase();   // 大文字小文字を区別しない
};

/**
 * 企業名類似判定
 * 法人格・スペース除去後、3文字以上が一方に含まれる → 類似
 *
 * 例:「杏林堂薬局」⊂「株式会社杏林堂薬局」→ ⚠️
 *    「ネッツトヨタ鹿児島」⊂「ネッツトヨタ鹿児島株式会社」→ ⚠️
 *    「二木」（2文字）→ スキップ
 */
...
/* ══════════════════════════════════════════════
   担当者名 正規化マッピング
   ニックネーム・旧表記・フルネーム → アプリ内正式名（苗字のみ）
   CSV/Excel インポート時にフルネームが入っても正しく名寄せする
══════════════════════════════════════════════ */
...
/**
 * 担当者名を正規化（CSV/Excel 取り込み時のフルネーム対応）
 *
 * 1. NAME_MAP に完全一致 → 変換
 * 2. スペースを半角統一して再マッチ
 * 3. スペースを除去して再マッチ（「上浦諒大」→「上浦」）
 * 4. 「姓 名」形式 → 姓（最初のトークン）だけ返す
 */
export const normalizeName = (name) => {
  ...
  /* ② スペース正規化（全角→半角）して再マッチ */
  const n = s.replace(/[\s　]+/g, " ").trim();
  if (NAME_MAP[n]) return NAME_MAP[n];

  /* ③ スペース除去して再マッチ（「早川 健晴」→「早川健晴」→ map） */
  const ns = n.replace(/\s/g, "");
  if (NAME_MAP[ns]) return NAME_MAP[ns];

  /* ④ 「姓 名」形式フォールバック：スペース前の姓だけ返す */
